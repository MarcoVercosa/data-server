...
const administratorGroupRoute = Router()
administratorGroupRoute.post("/administratorgroup", async (request: Request, response: Response) => {
    response.setHeader("Content-Type", "text/html; charset=utf-8");
    try {
        const { server, user, pass }: IParamsBody = request.body
        const scriptCommand = PowerShell.command`src/scripts/administratorGroup/start.ps1 ${server} ${user} ${pass}`
...
export { administratorGroupRoute }